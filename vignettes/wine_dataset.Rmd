---
title: "Crow - Wine Dataset Vignette"
...
```{r}
scr <- Crow::stochastic_clustering_runner(
  data_scaled,
  kmeans,
  labels_name = "cluster",
  n_runs = 30,
...
Let's visualise the majority voting labels returned by `Crow`.
```{r}
...
We can use `Crow`'s `parameter_searcher` function to find optimal clustering parameter values that result in reproducible and robust clustering results.
```{r}
param_grid = list(centers=seq(3,8), algorithm = c("Hartigan-Wong", "Lloyd", "Forgy", "MacQueen"))

parameter_searcher <- Crow::parameter_searcher(
  data_scaled,
  kmeans,
  labels_name = "cluster",
...
Crow::plot_heatmap(parameter_searcher$results_df, "centers", "algorithm")
```
...
```{r}
set.seed(42)
genetic_fs <- Crow::genetic_algorithm_feature_selector(
  data_scaled,
  kmeans,
  labels_name = "cluster",
  verbose = T,
...
Crow::plot_ga_fitness_evolution(genetic_fs$history)
```
